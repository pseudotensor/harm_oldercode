...
double vchar(double *pr, int js)
{
	...
	*/
	...
	/* root finding bit */
	/* bracket */
        dv = sqrt(cms2)/sqrt(fabs(Gcov[js][js]) + 1.) ;   /* guess; not totally general */
        v1 = (-v0 > 0.) ? -v0 + (1.-SAFE)*dv : -v0 - (1.-SAFE)*dv ;
        v2 = (-v0 > 0.) ? -v0 + (1.+SAFE)*dv : -v0 - (1.+SAFE)*dv ;
        dv *= SAFE ;
        err1 = chk_disp(v1) ;
        err2 = chk_disp(v2) ;
...
double chk_disp(double v) 
{
	...
	disp = c1*k2 + c2*kdB2 + sqrt(fabs(c3*k2*k2 + c4*k2*kdB2 + c5*kdB2*kdB2) + SMALL) ;
...
void coeff_set(double rho, double u)
{
	...
	c2 = 0.5*cs2/EE ;
	c3 = 0.25*cms2*cms2 ;
	c4 = 0.5*cs2*(cs2*EF + (va2 - 2.)*EE)/(EE*EE) ;
